...
def preprocess_image(img):
    ...
    image_unit8_array = image_unit8_array.reshape(actual_stream_width,h,3)
...
def output_processing(results, WIN_W, WIN_H):
    order = results.argsort()
    nwCat = gNetworkCategories()
    last = len(nwCat)-1
    text = gNetworkCategories[order[last-0]] + ' (' + '{0:.2f}'.format(output[order[last-0]]*100) + '%) '
    # Get text coordinates
    text_size = cv2.getTextSize(text, cv2.FONT_HERSHEY_SIMPLEX, TEXT_SIZE, 1)[0]
    text_coord_w = int( (WIN_W - text_size[0]) / 2 )
    text_coord_h = WIN_H - text_size[1]
    # Get box coordinates
    box_left_top_w = 0
    box_left_top_h = WIN_H - text_size[1]*3
    return text,(text_coord_w, text_coord_h), (box_left_top_w, box_left_top_h)
...
def main():
    ...
    while (True):
        ...
        res_label, e_time = perform_inference(input_img, exec_net, input_blob, output_blob)
